...
import { Instagram, Twitter, Youtube, Edit, Users, Star } from "lucide-react";
...
export default function ProfileViewPage() {
  ...
  if (loading || status === "loading")
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
        <p className="text-gray-400 text-lg">Cargando tu perfil...</p>
      </div>
    );
  ...
  if (!session)
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
        <p className="text-red-400 text-lg">No se pudo cargar tu perfil.</p>
      </div>
    );

  const equippedAchievement = profile?.equippedAchievement?.achievement;
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative z-10 bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl rounded-2xl p-8 w-full max-w-lg text-white">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-6">
          <div className="flex-shrink-0">
            <img
              src={profile?.image || "/images/defaults/avatar1.png"}
              alt="Avatar"
          ...
          <div className="flex flex-col items-center sm:items-start w-full">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => router.push(`/profile`)} // Te lleva a la página de edición
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-xl transition-colors shadow-md flex items-center justify-center gap-2"
              >
        ...
        <div className="grid grid-cols-3 gap-4 text-center bg-white/10 p-4 rounded-xl mb-6">
          ...
          <div>
            <p className="text-2xl font-bold">{profile?.weight || "-"}<span className="text-lg">kg</span></p>
            <p className="text-xs text-gray-300 uppercase">Peso</p>
          </div>
          <div>
            <p className="text-2xl font-bold">{profile?.height || "-"}<span className="text-lg">cm</span></p>
        ...
        <div className="space-y-4 mb-6">
          <div>
            ...
            <p className="text-gray-300 italic text-sm bg-white/5 p-3 rounded-lg">{profile?.bio || "Aún no has añadido una bio. ¡Edita tu perfil para añadirla!"}</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h3 className="font-semibold text-gray-200 mb-1">Objetivo</h3>
              <p className="text-gray-300 text-sm bg-white/5 p-3 rounded-lg">{profile?.goal || "No especificado"}</p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-200 mb-1">Nivel</h3>
              <p className="text-gray-300 text-sm bg-white/5 p-3 rounded-lg">{profile?.level || "No especificado"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center items-center gap-4">
          {profile?.socials?.instagram && (
            <a href={`https://instagram.com/${profile.socials.instagram}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-pink-500 transition"><Instagram className="w-6 h-6" /></a>
          )}
          {profile?.socials?.x && (
            <a href={`https://x.com/${profile.socials.x}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-400 transition"><Twitter className="w-6 h-6" /></a>
          )}
          {profile?.socials?.youtube && (
            <a href={`https://youtube.com/channel/${profile.socials.youtube}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-red-500 transition"><Youtube className="w-6 h-6" /></a>
          )}
        </div>
      </motion.div>
    </div>
